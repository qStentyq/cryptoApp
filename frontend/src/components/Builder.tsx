...
const BuilderEx2 = () => {
  ...
  const card2: BuilderInterface = {
    title:'HOT project',
    description: 'NEAR — продукт блокчейн-платформы NEAR Protocol, которая была запущена в октябре 2020 года с целью облегчить создание и использование децентрализованных приложений (dApps) в глобальном масштабе. Этот проект стремится решить проблему масштабируемости и производительности в блокчейне, чтобы ускорить принятие криптовалют. HOT — это центральный элемент экосистемы NEAR внутри Telegram. Благодаря мета-транзакциям использование HOT позволяет совершать реальные транзакции на блокчейне, играть в игры и оплачивать переводы.',
  ...
  const card3: BuilderInterface = {
    title:'Starknet',
    description: 'Starknet — это не требующий разрешений Validity-Rollup (также известный как «ZK-Rollup»), который поддерживает общие вычисления и в настоящее время работает как сеть L2 через Ethereum. Возможная безопасность L1 Starknet обеспечивается за счет использования самой безопасной и масштабируемой системы криптографической защиты — STARK. Контракты Starknet (по большей части) написаны на языке Cairo — полном языке программирования Тьюринга, разработанном для доказательств STARK. Токен Starknet будет использоваться для оплаты транзакционных сборов, участия в протоколе Starknet и участия в управлении.',
    image:  'https://miro.medium.com/v2/resize:fit:1358/1*Hr2JqtoQfU-CA-GL01haxA.jpeg',
    url: 'https://www.starknet.io/en'
  }
  const card4: BuilderInterface = {
    title:'Arbuz Token',
    description: 'Токен $ARBUZ - это новый мемкоин, криптовалюта, основанная на блокчейн-сети TON (Telegram Open Network). На данный момент курс $ARBUZ/USDT составляет приблизительно 3 доллара, и график показывает положительную динамику с момента запуска.',
